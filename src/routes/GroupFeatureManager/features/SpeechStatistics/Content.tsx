 ...
 */
const SpeechStatisticsGroupContent: React.FC<Props> = ({
  open,
  bot,
  group,
}) => {
    ...
    <>
      ...
      {config ? (
        <Descriptions bordered size="small" column={1}>
          <Descriptions.Item label="最小统计字数">
            {config.minSpeechLength} 字
          </Descriptions.Item>
          ...
          <Descriptions.Item label="排行榜奖励">
            {config.enableActivityReward ? (
              <Tag color="blue">
                {CYCLE_LABEL[config.activityRewardCycle] || "每日"} 前{" "}
                {config.activityRewardTopN} 名 +{config.activityRewardPoints}{" "}
                积分
              </Tag>
            ) : (
          ...
          <Descriptions.Item label="即时发言奖励">
            {config.enableSpeechReward ? (
              <Tag color="purple">
                {CYCLE_LABEL[config.speechRewardCycle] || "每日"} 每次 +
                {config.speechRewardPoints} 积分 （上限{" "}
                {config.speechRewardMaxTimes} 次）
              </Tag>
            ) : (
              <Tag color="default">未启用</Tag>
            )}
